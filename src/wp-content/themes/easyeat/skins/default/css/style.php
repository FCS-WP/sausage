...
if ( ! function_exists( 'easyeat_customizer_get_css' ) ) {

	function easyeat_customizer_get_css( $args = array() ) {
		...
		if ( $fonts ) {
			...
			$fonts        = easyeat_customizer_add_theme_fonts( $fonts );
			$rez          = array();
...
.easyeat_tabs .easyeat_tabs_titles li a {
...
CSS;
			$rez          = apply_filters( 'easyeat_filter_get_css', $rez, array( 'fonts' => $fonts ) );
			$css['fonts'] = $rez['fonts'];
		}
		...
		if ( $vars ) {
			...
			$vars = easyeat_customizer_add_theme_vars( $vars );

			$rez         = array();
			$rez['vars'] = '';
			$rez         = apply_filters( 'easyeat_filter_get_css', $rez, array( 'vars' => $vars ) );
			$css['vars'] = $rez['vars'];
		}
		...
		//--------------------------------------
		if ( false !== $colors ) {
			$schemes = empty( $scheme ) ? array_keys( easyeat_get_sorted_schemes() ) : array( $scheme );
			if ( count( $schemes ) > 0 ) {
				$rez = array();
				foreach ( $schemes as $s ) {
					// Prepare colors
					if ( empty( $scheme ) ) {
						$colors = easyeat_get_scheme_colors( $s );
					}
					...
					$rez['colors']  = '';
					$rez            = apply_filters(
						'easyeat_filter_get_css', $rez, array(
							'colors' => $colors,
							'scheme' => $s,
						)
					);
		...
		return apply_filters( 'easyeat_filter_prepare_css', $css_str, $remove_spaces );
	}
}
