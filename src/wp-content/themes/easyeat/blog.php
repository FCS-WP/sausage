...
if ( function_exists( 'easyeat_elementor_is_preview' ) && easyeat_elementor_is_preview() ) {
	...
	if ( have_posts() ) {
		...
		if ( isset( $GLOBALS['post'] ) && is_object( $GLOBALS['post'] ) ) {
			easyeat_storage_set( 'blog_archive_template_post', $GLOBALS['post'] );
		}
	}

	// Make a new main query
	easyeat_new_main_query(
		array(
			'post_type'      => easyeat_get_theme_option( 'post_type' ),
			'category'       => easyeat_get_theme_option( 'parent_cat' ),
	...
	get_template_part( apply_filters( 'easyeat_filter_get_template_part', easyeat_blog_archive_get_template() ) );
}
