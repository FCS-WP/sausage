<div class="front_page_section front_page_section_features<?php
	$easyeat_scheme = easyeat_get_theme_option( 'front_page_features_scheme' );
	if ( ! empty( $easyeat_scheme ) && ! easyeat_is_inherit( $easyeat_scheme ) ) {
		echo ' scheme_' . esc_attr( $easyeat_scheme );
	}
	echo ' front_page_section_paddings_' . esc_attr( easyeat_get_theme_option( 'front_page_features_paddings' ) );
	if ( easyeat_get_theme_option( 'front_page_features_stack' ) ) {
		echo ' sc_stack_section_on';
	}
?>"
		<?php
		$easyeat_css      = '';
		$easyeat_bg_image = easyeat_get_theme_option( 'front_page_features_bg_image' );
...
<?php
	// Add anchor
	$easyeat_anchor_icon = easyeat_get_theme_option( 'front_page_features_anchor_icon' );
	$easyeat_anchor_text = easyeat_get_theme_option( 'front_page_features_anchor_text' );
...
?>
	...
	<?php
	if ( easyeat_get_theme_option( 'front_page_features_fullheight' ) ) {
	...
	?>
			...
			<?php
			$easyeat_css      = '';
			$easyeat_bg_mask  = easyeat_get_theme_option( 'front_page_features_bg_mask' );
			$easyeat_bg_color_type = easyeat_get_theme_option( 'front_page_features_bg_color_type' );
			if ( 'custom' == $easyeat_bg_color_type ) {
				$easyeat_bg_color = easyeat_get_theme_option( 'front_page_features_bg_color' );
	...
	>
		<div class="front_page_section_content_wrap front_page_section_features_content_wrap content_wrap">
			<?php
			// Caption
			$easyeat_caption = easyeat_get_theme_option( 'front_page_features_caption' );
			if ( ! empty( $easyeat_caption ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<h2 class="front_page_section_caption front_page_section_features_caption front_page_block_<?php echo ! empty( $easyeat_caption ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( $easyeat_caption, 'easyeat_kses_content' ); ?></h2>
				<?php
			}

			// Description (text)
			$easyeat_description = easyeat_get_theme_option( 'front_page_features_description' );
			if ( ! empty( $easyeat_description ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<div class="front_page_section_description front_page_section_features_description front_page_block_<?php echo ! empty( $easyeat_description ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( wpautop( $easyeat_description ), 'easyeat_kses_content' ); ?></div>
				<?php
			}
			...
			<div class="front_page_section_output front_page_section_features_output">
				...
				if ( is_active_sidebar( 'front_page_features_widgets' ) ) {
					...
					if ( ! easyeat_exists_trx_addons() ) {
						...
						easyeat_customizer_need_widgets_message( 'front_page_features_caption', 'ThemeREX Addons - Services' );
					}
				}
				?>
			</div>
