...
jQuery( window ).on( 'load', function() {
	...
	if ( $editor_wrapper.length ) {
		...
		if ( typeof window.MutationObserver !== 'undefined' ) {
			...
			easyeat_create_observer( 'easyeat-check-visual-editor-wrapper', $editor_wrapper, function( mutationsList ) {
				easyeat_gutenberg_editor_init();
			} );
		} else {
		 	easyeat_gutenberg_editor_init();
		}
	...
	function easyeat_gutenberg_editor_object() {
		var editor = {
			$editor: false,
			$frame: false,
			...
			if ( $editor.length ) {
				...
				if ( editor_type == 'site' ) {
					...
					if ( editor.$frame.length && editor.$frame.get(0).contentDocument ) {
						...
						if ( editor.$styles_wrapper.hasClass( 'easyeat_inited' ) ) {
					...
					if ( ! editor.$editor.hasClass( 'easyeat_inited' ) ) {
	...
	// Init on page load
	function easyeat_gutenberg_editor_init() {

		// Get Gutenberg editor object
		var editor = easyeat_gutenberg_editor_object();
		if ( ! editor.$editor ) {
			return;
		}
		...
		function add_class_with_color_scheme( $obj ) {
			if ( ! $obj.hasClass( 'scheme_' + EASYEAT_STORAGE['color_scheme'] ) ) {
				$obj.addClass( 'scheme_' + EASYEAT_STORAGE['color_scheme'] );
			}
		}

		function add_class_with_overridden_options( $obj ) {
			for ( var i in EASYEAT_STORAGE['override_classes'] ) {
				$obj.addClass( EASYEAT_STORAGE['override_classes'][i].replace( '%s', EASYEAT_STORAGE[ i ] ) );
			}
		}
		...
		if ( editor_type == 'post' && editor.$styles_wrapper ) {
			...
			editor.$styles_wrapper.addClass( easyeat_get_class_by_prefix( editor.$editor.attr( 'class' ), 'post-type-' ) );
			...
			if ( editor.$writing_flow && EASYEAT_STORAGE['sidebar_position'] != 'hide' ) {
		...
		if ( editor_type == 'site' && editor.$styles_wrapper ) {
			...
			add_class_with_overridden_options( editor.$styles_wrapper );
			// Mark as inited
			editor.$styles_wrapper.addClass( 'easyeat_inited' );
		}
		...
		if ( editor_type == 'widgets' && editor.$writing_flow ) {
			...
			easyeat_remove_observer( 'easyeat-check-editor-styles-wrapper' );
			easyeat_create_observer( 'easyeat-check-editor-styles-wrapper', editor.$writing_flow, function( mutationsList ) {
		...
		editor.$editor.addClass( 'easyeat_inited' );
	}
} );
