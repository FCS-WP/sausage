...
jQuery( document ).ready(
	function() {
		...
		easyeat_add_filter('trx_addons_filter_sc_classes', function(classes, settings) {
		...
		setTimeout( function() {
			if ( window.elementor !== undefined ) {
				...
				jQuery( '#elementor-panel' )
					.on( 'input change', '[data-setting^="easyeat_options_"]', function (e) {
						...
						var link = jQuery( this ).parents( '.elementor-control' ).find( 'a.easyeat_post_editor' );
						if ( link.length > 0 ) {
							easyeat_change_post_edit_link_elementor( link );
						}
					} )
					.on( 'click', '.easyeat_post_editor', function(e) {
						easyeat_change_post_edit_link_elementor( jQuery(this) );
						if (jQuery(this).hasClass('easyeat_hidden' )) {
							e.preventDefault();
							return false;
						}
					});
		...
		function easyeat_change_post_edit_link_elementor(a) {
			if (a.length > 0) {
