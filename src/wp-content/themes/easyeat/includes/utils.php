...
if ( ! function_exists( 'easyeat_array_get_first' ) ) {
	/**
	 ...
	 */
	function easyeat_array_get_first( &$arr, $key = true ) {
		foreach ( $arr as $k => $v ) {
			break;
		}
		return $key ? $k : $v;
...
if ( ! function_exists( 'easyeat_array_get_keys_by_value' ) ) {
	/**
	 ...
	 */
	function easyeat_array_get_keys_by_value( $arr, $value = 1 ) {
		if ( ! is_array( $arr ) ) {
...
if ( ! function_exists( 'easyeat_array_delete_by_value' ) ) {
	/**
	 ...
	 */
	function easyeat_array_delete_by_value( $arr, $value ) {
...
if ( ! function_exists( 'easyeat_array_from_list' ) ) {
	/**
	 ...
	 */
	function easyeat_array_from_list( $arr ) {
		$new = array();
		foreach ( $arr as $v ) {
			$new[ $v ] = $v;
		}
...
if ( ! function_exists( 'easyeat_array_slice' ) ) {
	/**
	 ...
	 */
	function easyeat_array_slice( $arr, $from, $to='' ) {
...
if ( ! function_exists( 'easyeat_array_merge' ) ) {
	/**
	 * Merge arrays and lists (preserve a numeric indexes).
	 *
	 * For example:
	 *
	 * easyeat_array_merge( [ 1, 2, 3, 4 ], [ 'one', 'two' ] )  // result: [ 'one', 'two', 3, 4 ]
	 *
	 * easyeat_array_merge( [ 'a' => 'A', 'b' => 'B' ], [ 'one', 'two', 'a' => 'AA' ] )  // result: [ 'a' => 'AA', 'b' => 'B', 0 => 'one', 1 => 'two' ]
	 *
	 ...
	 */
	function easyeat_array_merge( $a1, $a2 ) {
...
if ( ! function_exists( 'easyeat_array_insert_after' ) ) {
	/**
	 ...
	 */
	function easyeat_array_insert_after( &$haystack, $needle, $stuff ) {
		...
		for ( $i = 2; $i < func_num_args(); ++$i ) {
			...
			if ( is_array( $arg ) ) {
				if ( 2 == $i ) {
					...
					$new_array = easyeat_array_merge( $new_array, $arg );
				}
			} else {
				$new_array[] = $arg;
			}
		...
		$haystack = is_int( $needle )
						...
						: easyeat_array_merge( array_slice( $haystack, 0, $i, true ), $new_array, array_slice( $haystack, $i, null, true ) );

		return $i;
	}
}

if ( ! function_exists( 'easyeat_array_insert_before' ) ) {
	/**
	 * Inserts any number of scalars or arrays at the point in the haystack
	 ...
	 * @return int               The index at which $needle was found.
	 */
	function easyeat_array_insert_before( &$haystack, $needle, $stuff ) {
		if ( ! is_array( $haystack ) ) {
			return -1;
		}
...
if ( ! function_exists( 'easyeat_array_get_by_key' ) ) {
	/**
	 ...
	 */
	function easyeat_array_get_by_key( &$arr, $key, $value, $key2 = '', $default = '' ) {
		$rez = $default;
		foreach ( $arr as $v ) {
...
if ( ! function_exists( 'easyeat_array_str_replace' ) ) {
	/**
	 ...
	 */
	function easyeat_array_str_replace( $from, $to, $key, $arr ) {
		foreach ( $arr as $k => $v ) {
			if ( is_array( $v ) ) {
				$arr[ $k ] = easyeat_array_str_replace( $from, $to, $key, $v );
			} else if ( $k == $key ) {
...
if ( ! function_exists( 'easyeat_generate_id' ) ) {
	/**
	 ...
	 */
	function easyeat_generate_id( $prefix = '' ) {
		return $prefix . str_replace( '.', '', mt_rand() );
	}
}

if ( ! function_exists( 'easyeat_get_tag' ) ) {
	/**
	 ...
	 */
	function easyeat_get_tag( $text, $tag_start, $tag_end = '' ) {
		$val       = '';
...
if ( ! function_exists( 'easyeat_get_tag_attrib' ) ) {
	/**
	 ...
	 */
	function easyeat_get_tag_attrib( $text, $tag, $attr ) {
		$val       = '';
...
if ( ! function_exists( 'easyeat_get_css_position_from_values' ) ) {
	/**
	 ...
	 */
	function easyeat_get_css_position_from_values( $top = '', $right = '', $bottom = '', $left = '', $width = '', $height = '' ) {
		if ( ! is_array( $top ) ) {
		...
		foreach ( $top as $k => $v ) {
			...
			if ( '' != $v ) {
				$output .= ( 'width' == $k ? 'width' : ( 'height' == $k ? 'height' : 'margin-' . esc_attr( $k ) ) ) . ':' . esc_attr( easyeat_prepare_css_value( $v ) ) . ( '!' == $imp ? ' !important' : '' ) . ';';
			}
		}
		return $output;
	}
...
if ( ! function_exists( 'easyeat_prepare_css_value' ) ) {
	/**
	 ...
	 */
	function easyeat_prepare_css_value( $val ) {
		if ( '' !== $val ) {
...
if ( ! function_exists( 'easyeat_parse_icons_classes' ) ) {
	/**
	 ...
	 */
	function easyeat_parse_icons_classes( $css ) {
		$rez = array();
		if ( ! file_exists( $css ) ) {
			return $rez;
		}
		$file = easyeat_fga( $css );
...
if ( ! function_exists( 'easyeat_get_column_class' ) ) {
	/**
	 ...
	 */
	function easyeat_get_column_class( $num, $all, $all_tablet = '', $all_mobile = '' ) {
...
if ( ! function_exists( 'easyeat_stripslashes' ) ) {
	/**
	 ...
	 */
	function easyeat_stripslashes( $val ) {
		static $magic = 0;
		if ( 0 === $magic ) {
...
if ( ! function_exists( 'easyeat_get_value_gp' ) ) {
	/**
	 ...
	 */
	function easyeat_get_value_gp( $name, $defa = '' ) {
		if ( isset( $_GET[ $name ] ) ) {
...
if ( ! function_exists( 'easyeat_get_value_gpc' ) ) {
	/**
	 ...
	 */
	function easyeat_get_value_gpc( $name, $defa = '' ) {
		if ( isset( $_GET[ $name ] ) ) {
			$rez = wp_unslash( $_GET[ $name ] );
		} elseif ( isset( $_POST[ $name ] ) ) {
			$rez = wp_unslash( $_POST[ $name ] );
...
if ( ! function_exists( 'easyeat_get_value_gps' ) ) {
	/**
	 ...
	 */
	function easyeat_get_value_gps( $name, $defa = '' ) {
		global $wp_session;
		if ( isset( $_GET[ $name ] ) ) {
			$rez = wp_unslash( $_GET[ $name ] );
		} elseif ( isset( $_POST[ $name ] ) ) {
...
if ( ! function_exists( 'easyeat_get_session_value' ) ) {
	/**
	 ...
	 */
	function easyeat_get_session_value( $name, $defa = '' ) {
		global $wp_session;
		return isset( $wp_session[ $name ] ) ? $wp_session[ $name ] : $defa;
	}
}

if ( ! function_exists( 'easyeat_set_session_value' ) ) {
	/**
	 ...
	 */
	function easyeat_set_session_value( $name, $value ) {
		global $wp_session;
		$wp_session[ $name ] = $value;
	}
}

if ( ! function_exists( 'easyeat_del_session_value' ) ) {
	/**
	 * Delete a saved variable from the current session.
	 *
	 * @param string $name   A name to to delete from the session.
	 */
	function easyeat_del_session_value( $name ) {
		global $wp_session;
		unset( $wp_session[ $name ] );
	}
}

if ( ! function_exists( 'easyeat_get_cookie' ) ) {
	/**
	 ...
	 */
	function easyeat_get_cookie( $name, $defa = '' ) {
		return easyeat_stripslashes( isset( $_COOKIE[ $name ] ) ? $_COOKIE[ $name ] : $defa );
	}
}

// Set a cookie - wrapper for setcookie using WP constants.
if ( ! function_exists( 'easyeat_set_cookie' ) ) {
	/**
	 ...
	 */
	function easyeat_set_cookie( $name, $value, $expire = 0, $secure = false, $httponly = false  ) {
		if ( ! headers_sent() ) {
			if ( defined( 'PHP_VERSION_ID' ) && PHP_VERSION_ID >= 70300 ) {
				$rez = setcookie(
							$name,
							$value,
							apply_filters( 'easyeat_filter_cookie_options', array(
								'expires'  => $expire,
								'path'     => defined( 'COOKIEPATH' ) && ! empty( COOKIEPATH ) ? COOKIEPATH : '/',
								'domain'   => defined( 'COOKIE_DOMAIN' ) && ! empty( COOKIE_DOMAIN ) ? COOKIE_DOMAIN : '',	//easyeat_get_domain_from_url( get_home_url() ),
								'secure'   => $secure,
								'httponly' => $httponly,
				...
				$rez = setcookie(
							...
							defined( 'COOKIE_DOMAIN' ) && ! empty( COOKIE_DOMAIN ) ? COOKIE_DOMAIN : '',	//easyeat_get_domain_from_url( get_home_url() ),
							$secure,
							$httponly
						);
			}
			...
			trigger_error( sprintf( __( '%1$s cookie cannot be set - headers already sent by %2$s on line %3$s', 'easyeat' ), $name, $file, $line ), E_USER_NOTICE ); // @codingStandardsIgnoreLine
		}
	}
}
...
if ( ! function_exists( 'easyeat_hex2rgb' ) ) {
	/**
...
if ( ! function_exists( 'easyeat_hex2rgba' ) ) {
	/**
	 ...
	 */
	function easyeat_hex2rgba( $hex, $alpha ) {
		$rgb = easyeat_hex2rgb( $hex );
		return 'rgba(' . intval( $rgb['r'] ) . ',' . intval( $rgb['g'] ) . ',' . intval( $rgb['b'] ) . ',' . floatval( $alpha ) . ')';
	}
}

if ( ! function_exists( 'easyeat_hex2hsb' ) ) {
	/**
	 ...
	 */
	function easyeat_hex2hsb( $hex, $h = 0, $s = 0, $b = 0 ) {
		$hsb      = easyeat_rgb2hsb( easyeat_hex2rgb( $hex ) );
...
if ( ! function_exists( 'easyeat_rgb2hsb' ) ) {
	/**
...
if ( ! function_exists( 'easyeat_hsb2rgb' ) ) {
	/**
...
if ( ! function_exists( 'easyeat_rgb2hex' ) ) {
	/**
...
if ( ! function_exists( 'easyeat_hsb2hex' ) ) {
	/**
	 ...
	 * @return string     A string with a hex color presentation in the format '#RRGGBB'.
	 */
	function easyeat_hsb2hex( $hsb ) {
		return easyeat_rgb2hex( easyeat_hsb2rgb( $hsb ) );
	}
}
...
//
if ( ! function_exists( 'easyeat_date_to_sql' ) ) {
	/**
	 ...
	 */
	function easyeat_date_to_sql($str) {
		if (trim($str)=='') return '';
		$str = strtr(trim($str),'/\.,','----');
...
if ( ! function_exists( 'easyeat_format_price' ) ) {
	/**
	 ...
	 */
	function easyeat_format_price( $price ) {
...
if ( ! function_exists( 'easyeat_num2size' ) ) {
	/**
	 ...
	 */
	function easyeat_num2size( $num ) {
		return $num > 1000 ? round( $num/1000, 0 ) . 'K' : $num;
	}
}

if ( ! function_exists( 'easyeat_size2num' ) ) {
	/**
	 ...
	 */
	function easyeat_size2num( $size, $full = false ) {
...
//
if ( ! function_exists( 'easyeat_parse_num' ) ) {
	/**
	 ...
	 */
	function easyeat_parse_num( $str ) {
...
if ( ! function_exists( 'easyeat_prepare_macros' ) ) {
	/**
	 ...
	 * @return string      A processed string.
	 */
	function easyeat_prepare_macros( $str ) {
		return str_replace(
			array( '{{', '}}', '((', '))', '||' ),
			array( '<i>', '</i>', '<b>', '</b>', '<br>' ),
			$str
...
if ( ! function_exists( 'easyeat_remove_macros' ) ) {
	/**
	 ...
	 * @return string      A processed string.
	 */
	function easyeat_remove_macros( $str ) {
		return str_replace(
			array( '{{', '}}', '((', '))', '||' ),
			array( '', '', '', '', ' ' ),
			$str
...
if ( ! function_exists( 'easyeat_is_on' ) ) {
	/**
...
if ( ! function_exists( 'easyeat_is_off' ) ) {
	/**
...
if ( ! function_exists( 'easyeat_is_inherit' ) ) {
	/**
	 ...
	 */
	function easyeat_is_inherit( $prm ) {
		return ! is_array( $prm ) && in_array( strtolower( $prm ), array( 'inherit' ) );
	}
}

if ( ! function_exists( 'easyeat_strshort' ) ) {
	/**
	 ...
	 */
	function easyeat_strshort( $str, $maxlength, $add = '&hellip;' ) {
		if ( 0 >= $maxlength ) {
			return '';
		}
		$str = easyeat_strip_tags( $str );
...
if ( ! function_exists( 'easyeat_strwords' ) ) {
	/**
	 ...
	 * @return string            A truncated string.
	 */
	function easyeat_strwords( $str, $maxlength, $add = '&hellip;' ) {
		if ( $maxlength <= 0 ) {
			return '';
		}
		$words = explode( ' ', $str );
...
if ( ! function_exists( 'easyeat_strip_tags' ) ) {
	/**
	 ...
	 */
	function easyeat_strip_tags( $str ) {
...
if ( ! function_exists( 'easyeat_excerpt' ) ) {
	/**
	 ...
	 */
	function easyeat_excerpt( $str, $maxlength, $add = '&hellip;' ) {
		if ( $maxlength <= 0 ) {
			return '';
		}
		return easyeat_strwords( easyeat_strip_tags( $str ), $maxlength, $add );
	}
}

if ( ! function_exists( 'easyeat_unserialize_recover' ) ) {
	/**
	 ...
	 * @return string      A processed string.
	 */
	function easyeat_unserialize_recover( $str ) {
...
if ( ! function_exists( 'easyeat_unserialize' ) ) {
	/**
	 ...
	 */
	function easyeat_unserialize( $str ) {
		if ( ! empty( $str ) && is_serialized( $str ) ) {
			...
			if ( true || ! preg_match( '/O:[0-9]+:"([^"]*)":[0-9]+:{/', $str, $matches ) || empty( $matches[1] ) || class_exists( $matches[1] ) ) {
				try {
					...
					if ( false === $data ) {
						$data = @unserialize( easyeat_unserialize_recover( $str ) );
					}
					// Attempt 4: try unserialize original string without CR symbol '\r' with modified character counters
					if ( false === $data ) {
						$data = @unserialize( easyeat_unserialize_recover( $str2 ) );
					}
				} catch ( Exception $e ) {
...
if ( ! function_exists( 'easyeat_str_replace' ) ) {
	/**
	 ...
	 */
	function easyeat_str_replace( $from, $to, $str ) {
		if ( is_array( $str ) ) {
			foreach ( $str as $k => $v ) {
				$str[ $k ] = easyeat_str_replace( $from, $to, $v );
			}
			...
			if ( is_serialized( $str ) ) {
				$str = serialize( easyeat_str_replace( $from, $to, easyeat_unserialize( $str ) ) );
			} else {
...
if ( ! function_exists( 'easyeat_str_replace_once' ) ) {
	/**
	 ...
	 * @return string             A processed string.
	 */
	function easyeat_str_replace_once( $from, $to, $str ) {
		$rez = '';
		if ( ! is_array( $from ) ) {
			$from = array( $from );
		}
...
if ( ! function_exists( 'easyeat_tags_count' ) ) {
	/**
	 ...
	 */
	function easyeat_tags_count( $str, $tag ) {
		$cnt = 0;
...
if ( ! function_exists( 'easyeat_getimagesize' ) ) {
	/**
	 ...
	 */
	function easyeat_getimagesize( $url, $echo = false ) {
		$img_size = false;
		if ( is_string( $url ) ) {
			$img_path = easyeat_is_url( $url ) ? easyeat_url_to_local_path( $url ) : $url;
...
if ( ! function_exists( 'easyeat_clear_thumb_size' ) ) {
	/**
	 ...
	 */
	function easyeat_clear_thumb_size( $url, $remove_protocol = true ) {
		if ( empty( $url ) ) return '';
		$pi = pathinfo( $url );
		if ( $remove_protocol ) {
			$pi['dirname'] = easyeat_remove_protocol_from_url( $pi['dirname'], false );
		}
...
if ( ! function_exists( 'easyeat_add_thumb_size' ) ) {
	/**
	 ...
	 */
	function easyeat_add_thumb_size( $url, $thumb_size, $check_exists = true ) {
		...
		$new_url = easyeat_remove_protocol_from_url( ( ! empty( $pi['dirname'] ) ? $pi['dirname'] . '/' : '' ) . $pi['filename'] . ( ! empty( $pi['extension'] ) ? '.' . $pi['extension'] : '' ) );

		// Check exists
		if ( $check_exists ) {
			$uploads_info = wp_upload_dir();
			$uploads_url = easyeat_remove_protocol_from_url( $uploads_info['baseurl'] );
			...
			if ( strpos( $new_url, $uploads_url ) !== false ) {
				if ( ! file_exists( str_replace( $uploads_url, $uploads_dir, $new_url ) ) ) {
					$new_url = easyeat_remove_protocol_from_url( $url );
				}
			} else {
				$new_url = easyeat_remove_protocol_from_url( $url );
			}
...
if ( ! function_exists( 'easyeat_get_thumb_size' ) ) {
	 ...
	 */
	function easyeat_get_thumb_size( $ts ) {
		$thumb_sizes = easyeat_storage_get( 'theme_thumbs' );
		if ( ! empty( $thumb_sizes[ "easyeat-thumb-{$ts}" ] ) ) {
			$ts = 'easyeat-thumb-' . $ts;
		}
		$retina = easyeat_get_retina_multiplier() > 1 ? '-@retina' : '';
		$is_external = apply_filters(
							'easyeat_filter_is_external_thumb_size',
							in_array( $ts,
										apply_filters(
											'easyeat_filter_external_thumb_sizes',
		...
		$is_internal = strpos( $ts, 'easyeat-thumb-' ) === 0 || strpos( $ts, 'trx_addons-thumb-' ) === 0;
		return apply_filters(
					'easyeat_filter_get_thumb_size',
					( $is_external || $is_internal ? '' : 'easyeat-thumb-' )
					. $ts
					. ( $is_internal ? $retina : '' )
				);
	}
...
if ( ! function_exists( 'easyeat_get_closest_thumb_size' ) ) {
	 ...
	 */
	function easyeat_get_closest_thumb_size( $old_size, $dim, $prefix = '' ) {
...
if ( ! function_exists( 'easyeat_get_attachment_url' ) ) {
	/**
	 ...
	 * @return string
	 */
	function easyeat_get_attachment_url( $image_id, $size = 'full' ) {
		...
		if ( is_numeric( $image_id ) && (int) $image_id > 0 ) {
			...
			$image_id = easyeat_add_thumb_size( $image_id, $size );
		}
		return $image_id;
	}
}

if ( ! function_exists( 'easyeat_get_post_image' ) ) {
	/**
	 ...
	 */
	function easyeat_get_post_image( $post_text = '', $src = true ) {
		global $post;
		$img = '';
		if ( empty( $post_text ) ) {
...
if ( ! function_exists( 'easyeat_get_post_audio' ) ) {
	/**
	 ...
	 * @return string           Founded URL (or a whole tag) or an empty string if found nothing.
	 */
	function easyeat_get_post_audio( $post_text = '', $src = true ) {
		global $post;
		$img = easyeat_get_post_audio_list_first( ! $src );
		if ( is_array( $img ) ) {
			if ( ! empty( $img['url'] ) ) {
				$img = $img['url'];
			} else if ( ! empty( $img['embed'] ) ) {
				$img = easyeat_get_post_iframe( $img['embed'], true );
			} else {
				$img = '';
			}
		}
		if ( empty( $img ) ) {
			...
			if ( $src ) {
				...
				$img = easyeat_get_tag( $post_text, '<audio', '</audio>' );
				if ( empty( $img ) ) {
					$img = do_shortcode( easyeat_get_tag( $post_text, '[audio', '[/audio]' ) );
				}
				if ( empty( $img ) ) {
					$img = easyeat_get_tag_attrib( $post_text, '[trx_widget_audio]', 'url' );
...
if ( ! function_exists( 'easyeat_get_post_audio_list' ) ) {
	/**
	 ...
	 *                     ]
	 */
	function easyeat_get_post_audio_list( $args=array() ) {
		return function_exists( 'trx_addons_get_post_audio_list' ) ? trx_addons_get_post_audio_list( $args ) : array();
	}
}

if ( ! function_exists( 'easyeat_get_post_audio_list_first' ) ) {
	/**
...
if ( ! function_exists( 'easyeat_get_post_video' ) ) {
	 ...
	 * @return string           Founded URL (or a whole tag) or an empty string if found nothing.
	 */
	function easyeat_get_post_video( $post_text = '', $src = true ) {
		global $post;
		$img = easyeat_get_post_video_list_first( ! $src );
		if ( is_array( $img ) ) {
			if ( ! empty( $img['video_url'] ) ) {
				$img = $img['video_url'];
			} else if ( ! empty( $img['video_embed'] ) ) {
				$img = easyeat_get_post_iframe( $img['video_embed'], true );
			} else {
				$img = '';
			}
		}
		...
		return apply_filters( 'easyeat_filter_get_post_video', $img );
	}
}

if ( ! function_exists( 'easyeat_get_post_video_list' ) ) {
	/**
	 ...
	 *                     ]
	 */
	function easyeat_get_post_video_list( $args=array() ) {
		return function_exists( 'trx_addons_get_post_video_list' ) ? trx_addons_get_post_video_list( $args ) : array();
	}
}

if ( ! function_exists( 'easyeat_get_post_video_list_first' ) ) {
	/**
	 ...
	 *                     or the URL only
	 */
	function easyeat_get_post_video_list_first( $src = false, $args = array() ) {
		return function_exists( 'trx_addons_get_post_video_list_first' ) ? trx_addons_get_post_video_list_first( $src, $args ) : '';
	}
}

if ( ! function_exists( 'easyeat_get_post_iframe' ) ) {
	/**
	 ...
	 * @return string           Founded URL (or a whole tag) or an empty string if found nothing.
	 */
	function easyeat_get_post_iframe( $post_text = '', $src = true ) {
		global $post;
		$img = '';
		$tag = easyeat_get_embed_video_tag_name();
		if ( empty( $post_text ) ) {
			$post_text = $post->post_content;
		}
		if ( $src ) {
			...
			$img = easyeat_get_tag( $post_text, '<' . esc_html( $tag ), '</' . esc_html( $tag ) . '>' );
		}
		return apply_filters( 'easyeat_filter_get_post_iframe', $img );
	}
}

if ( ! function_exists( 'easyeat_get_embed_video' ) ) {
	/**
	 ...
	 */
	function easyeat_get_embed_video( $video, $use_wp_embed = false, $args = array() ) {
		global $wp_embed;
		if ( $use_wp_embed && is_object( $wp_embed ) ) {
			$embed_video = do_shortcode( $wp_embed->run_shortcode( '[embed]' . trim( $video ) . '[/embed]' ) );
		} else if ( easyeat_is_from_uploads( $video ) ) {
			$embed_video  = '<video'
								. ' src="' . esc_url( $video ) . '"'
			...
			if ( strpos( $video, 'youtu.be/' ) !== false || strpos( $video, 'youtube.com/' ) !== false ) {
				...
				$video = easyeat_add_to_url( $video, easyeat_get_embed_video_parameters() );
			}
			...
			$ratio  = apply_filters( 'easyeat_filter_video_ratio', $is_tiktok ? '9:21' : '16:9' );
			$parts  = explode( ':', $ratio );
			$width  = $is_tiktok ? apply_filters( 'easyeat_filter_video_width', 325, 'tiktok' ) : easyeat_get_content_width();
			$height = round( $width / $parts[0] * $parts[1] );
			$dim    = apply_filters( 'easyeat_filter_video_dimensions', array(
									'width' => $width,
									'height' => $height
									), $ratio );
...
if ( ! function_exists( 'easyeat_get_embed_video_tag_name' ) ) {
	/**
	 * Return a name of the tag-container to embed video.
	 *
	 * @return string  A tag name to embed video.
	 */
	function easyeat_get_embed_video_tag_name() {
...
if ( ! function_exists( 'easyeat_make_video_autoplay' ) ) {
	/**
	 ...
	 */
	function easyeat_make_video_autoplay( $video, $muted = false ) {
					...
					if ( ! empty( $matches[2] ) ) {
						...
						$matches[2] = easyeat_add_to_url(
							...
							array_merge(
								...
								$muted
									? array_merge(
										easyeat_get_embed_video_parameters(),
										array(
											'muted' => 1,
											'controls' => 0,
											'background' => 1,
...
if ( ! function_exists( 'easyeat_is_from_uploads' ) ) {
	/**
	 ...
	 */
	function easyeat_is_from_uploads( $url ) {
		$url          = easyeat_remove_protocol_from_url( $url );
		$parts        = explode( '?', $url );
		$url          = $parts[0];
		$uploads_info = wp_upload_dir();
		$uploads_url  = easyeat_remove_protocol_from_url( $uploads_info['baseurl'] );
...
if ( ! function_exists( 'easyeat_is_youtube_url' ) ) {
	/**
	 ...
	 */
	function easyeat_is_youtube_url( $url ) {
		return strpos( $url, 'youtu.be' ) !== false || strpos( $url, 'youtube.com' ) !== false;
	}
}

if ( ! function_exists( 'easyeat_is_vimeo_url' ) ) {
	/**
	 ...
	 */
	function easyeat_is_vimeo_url( $url ) {
...
if ( ! function_exists( 'easyeat_init_filesystem' ) ) {
	add_action( 'after_setup_theme', 'easyeat_init_filesystem', 0 );
	/**
	 ...
	 */
	function easyeat_init_filesystem( $force = false ) {
...
if ( ! function_exists( 'easyeat_fpc' ) ) {
	/**
	 ...
	 */
	function easyeat_fpc( $file, $data, $flag = 0 ) {
		if ( ! empty( $file ) ) {
			if ( function_exists( 'trx_addons_fpc' ) ) {
				...
				if ( isset( $wp_filesystem ) && is_object( $wp_filesystem ) ) {
					...
					if ( easyeat_is_on( easyeat_get_theme_option( 'debug_mode' ) ) ) {
						...
						throw new Exception( sprintf( esc_html__( 'WP Filesystem is not initialized! Put contents to the file "%s" failed', 'easyeat' ), $file ) );
					}
				}
			}
		}
...
if ( ! function_exists( 'easyeat_fgc' ) ) {
	/**
	...
	function easyeat_fgc( $file ) {
		if ( ! empty( $file ) ) {
			if ( function_exists( 'trx_addons_fgc' ) ) {
				...
				global $wp_filesystem;
				if ( isset( $wp_filesystem ) && is_object( $wp_filesystem ) ) {
					$file = substr( $file, 0, 2 ) == '//' ? easyeat_add_protocol( $file ) : str_replace( ABSPATH, $wp_filesystem->abspath(), $file );
					return $wp_filesystem->get_contents( $file );
				} else {
					if ( easyeat_is_on( easyeat_get_theme_option( 'debug_mode' ) ) ) {
						// Translators: Add the file name to the message
						throw new Exception( sprintf( esc_html__( 'WP Filesystem is not initialized! Get contents from the file "%s" failed', 'easyeat' ), $file ) );
					}
				}
			}
		}
...
if ( ! function_exists( 'easyeat_fga' ) ) {
	/**
		...
		if ( ! empty( $file ) ) {
			if ( function_exists( 'trx_addons_fga' ) ) {
				...
				if ( isset( $wp_filesystem ) && is_object( $wp_filesystem ) ) {
					...
					if ( easyeat_is_on( easyeat_get_theme_option( 'debug_mode' ) ) ) {
						// Translators: Add the file name to the message
						throw new Exception( sprintf( esc_html__( 'WP Filesystem is not initialized! Get rows from the file "%s" failed', 'easyeat' ), $file ) );
					}
				}
			}
		}
...
if ( ! function_exists( 'easyeat_mkdir' ) ) {
	/**
				...
				if ( isset( $wp_filesystem ) && is_object( $wp_filesystem ) ) {
					...
					if ( ! $wp_filesystem->is_dir( $path ) ) {
						...
						if ( ! wp_mkdir_p( $path ) ) {
							if ( easyeat_is_on( easyeat_get_theme_option( 'debug_mode' ) ) ) {
								// Translators: Add the file name to the message
								throw new Exception( sprintf( esc_html__( 'Create a folder "%s" failed', 'easyeat' ), $path ) );
							}
						} else {
							return true;
						}
					...
					if ( easyeat_is_on( easyeat_get_theme_option( 'debug_mode' ) ) ) {
						// Translators: Add the file name to the message
						throw new Exception( sprintf( esc_html__( 'WP Filesystem is not initialized! Create a folder "%s" failed', 'easyeat' ), $path ) );
					}
				}
			}
		}
...
if ( ! function_exists( 'easyeat_unlink' ) ) {
	/**
	 ...
	 * @throws Exception        Throw an Exception if the object $wp_filesystem is not initialized yet or if error occurs.
	 */
	function easyeat_unlink( $path, $recursive = true, $type = 'd' ) {
		if ( ! empty( $path ) ) {
			if ( function_exists( 'trx_addons_unlink' ) ) {
				...
				if ( isset( $wp_filesystem ) && is_object( $wp_filesystem ) ) {
					...
					if ( easyeat_is_on( easyeat_get_theme_option( 'debug_mode' ) ) ) {
						// Translators: Add the file name to the message
						throw new Exception( sprintf( esc_html__( 'WP Filesystem is not initialized! Delete a file/folder "%s" failed', 'easyeat' ), $path ) );
					}
				}
			}
		}
...
if ( ! function_exists( 'easyeat_copy' ) ) {
	/**
	...
	function easyeat_copy( $src, $dst ) {
		...
		if ( ! empty( $src ) && ! empty( $dst ) ) {
			if ( function_exists( 'trx_addons_copy' ) ) {
				...
				$src = easyeat_prepare_path( $src );
				$dst = easyeat_prepare_path( $dst );
				...
				if ( is_dir( $src ) ) {
					...
					foreach ( $files as $file ) {
						if ( $file != "." && $file != ".." ) {
							easyeat_copy( "$src/$file", "$dst/$file" );
						}
					}
					return true;
...
if ( ! function_exists( 'easyeat_unzip_file' ) ) {
	/**
...
if ( ! function_exists( 'easyeat_esc' ) ) {
	 ...
	 */
	function easyeat_esc( $name ) {
		return str_replace( array( '\\', '~', '$', ':', ';', '+', '>', '<', '|', '"', "'", '`', "\xFF", "\x0A", "\x0D", '*', '?', '^' ), '/', trim( $name ) );
	}
}

if ( ! function_exists( 'easyeat_prepare_path' ) ) {
	/**
...
if ( ! function_exists( 'easyeat_check_min_file' ) ) {
	 ...
	 */
	function easyeat_check_min_file( $file, $dir = '' ) {
		if ( empty( $dir ) ) {
			$dir = dirname( $file );
		}
		if ( substr( $file, -3 ) == '.js' ) {
			if ( substr( $file, -7 ) != '.min.js' && easyeat_is_off( easyeat_get_theme_option( 'debug_mode' ) ) ) {
			...
			if ( substr( $file, -8 ) != '.min.css' && easyeat_is_off( easyeat_get_theme_option( 'debug_mode' ) ) ) {
				$dir      = trailingslashit( $dir );
...
if ( ! function_exists( 'easyeat_get_file_dir' ) ) {
	/**
	 ...
	 */
	function easyeat_get_file_dir( $file, $return_url = false ) {
		// Use new WordPress functions (if present)
		if ( function_exists( 'get_theme_file_path' ) && ! EASYEAT_ALLOW_SKINS && ! easyeat_get_theme_setting( 'check_min_version', false ) ) {
		...
		// Otherwise (on WordPress older then 4.7.0) or theme use .min versions of .js and .css or theme use skins
			...
			$dir       = '';
			$theme_dir = apply_filters( 'easyeat_filter_get_theme_file_dir', '', $file, $return_url );
			if ( '' != $theme_dir ) {
				$dir = $theme_dir;
			} elseif ( EASYEAT_CHILD_DIR != EASYEAT_THEME_DIR && file_exists( EASYEAT_CHILD_DIR . ( $file ) ) ) {
				$dir = ( $return_url ? EASYEAT_CHILD_URL : EASYEAT_CHILD_DIR ) . easyeat_check_min_file( $file, EASYEAT_CHILD_DIR );
			} elseif ( file_exists( EASYEAT_THEME_DIR . ( $file ) ) ) {
				$dir = ( $return_url ? EASYEAT_THEME_URL : EASYEAT_THEME_DIR ) . easyeat_check_min_file( $file, EASYEAT_THEME_DIR );
			}
		}
		return $dir;
	}
...
if ( ! function_exists( 'easyeat_get_file_url' ) ) {
	/**
	 ...
	 *                          or in the main theme directory. If a file is not exists - return an empty string.
	 */
	function easyeat_get_file_url( $file ) {
		return easyeat_get_file_dir( $file, true );
	}
}

if ( ! function_exists( 'easyeat_get_file_ext' ) ) {
	/**
	 ...
	 */
	function easyeat_get_file_ext( $file ) {
		$ext = pathinfo( $file, PATHINFO_EXTENSION );
		return empty( $ext ) ? '' : $ext;
	}
}

if ( ! function_exists( 'easyeat_get_file_name' ) ) {
	/**
	 ...
	 */
	function easyeat_get_file_name( $file, $without_ext = true ) {
		$parts = pathinfo( $file );
		return ! empty( $parts['filename'] ) && $without_ext ? $parts['filename'] : $parts['basename'];
	}
}

if ( ! function_exists( 'easyeat_get_folder_dir' ) ) {
	/**
	 ...
	 */
	function easyeat_get_folder_dir( $folder, $return_url = false ) {
		if ( '/' == $folder[0] ) {
			$folder = substr( $folder, 1 );
		}
		$dir       = '';
		$theme_dir = apply_filters( 'easyeat_filter_get_theme_folder_dir', '', $folder, $return_url );
		if ( '' != $theme_dir ) {
			$dir = $theme_dir;
		} elseif ( EASYEAT_CHILD_DIR != EASYEAT_THEME_DIR && is_dir( EASYEAT_CHILD_DIR . ( $folder ) ) ) {
			$dir = ( $return_url ? EASYEAT_CHILD_URL : EASYEAT_CHILD_DIR ) . ( $folder );
		} elseif ( is_dir( EASYEAT_THEME_DIR . ( $folder ) ) ) {
			$dir = ( $return_url ? EASYEAT_THEME_URL : EASYEAT_THEME_DIR ) . ( $folder );
		}
		return apply_filters( 'easyeat_filter_get_folder_dir', $dir, $folder, $return_url );
	}
}

if ( ! function_exists( 'easyeat_get_folder_url' ) ) {
	/**
	 ...
	 *                          or in the main theme directory. If a folder is not exists - return an empty string.
	 */
	function easyeat_get_folder_url( $folder ) {
		return easyeat_get_folder_dir( $folder, true );
	}
}

if ( ! function_exists( 'easyeat_merge_js' ) ) {
	/**
	 ...
	 */
	function easyeat_merge_js( $to, $list ) {
		$s = '';
		foreach ( $list as $f ) {
			$s .= easyeat_fgc( easyeat_get_file_dir( $f ) );
		}
		if ( '' != $s ) {
			$file_dir = easyeat_get_file_dir( $to );
			if ( empty( $file_dir ) && strpos( $to, '-full.js' ) !== false ) {
				$file_dir = easyeat_get_file_dir( str_replace( '-full.js', '.js', $to ) );
				if ( ! empty( $file_dir ) ) {
					$file_dir = str_replace( '.js', '-full.js', $file_dir );
				}
			}
			easyeat_fpc( $file_dir,
				'/* '
				. strip_tags( __( "ATTENTION! This file was generated automatically! Don't change it!!!", 'easyeat' ) )
				. "\n----------------------------------------------------------------------- */\n"
				. apply_filters( 'easyeat_filter_js_output', apply_filters( 'easyeat_filter_prepare_js', $s, true ), $to )
			);
		}
	}
}

if ( ! function_exists( 'easyeat_merge_css' ) ) {
	/**
	 ...
	 */
	function easyeat_merge_css( $to, $list, $need_responsive = false ) {
		if ( $need_responsive ) {
			$responsive = apply_filters( 'easyeat_filter_responsive_sizes', easyeat_storage_get( 'responsive' ) );
		}
		$sizes  = array();
		$output = '';
		foreach ( $list as $f ) {
			$fdir = easyeat_get_file_dir( $f );
			if ( '' != $fdir ) {
				$css = easyeat_fgc( $fdir );
				if ( $need_responsive ) {
					$pos = 0;
					while( false !== $pos ) {
						...
						if ( false !== $pos ) {
							...
							$sizes[ $media ] .= "\n\n" . apply_filters( 'easyeat_filter_merge_css', substr( $css, $pos_lbrace + 1, $pos_rbrace - $pos_lbrace - 1 ) );
							$css = substr( $css, $pos_rbrace + 1);
						}
					}
				} else {
					$output .= "\n\n" . apply_filters( 'easyeat_filter_merge_css', $css );
				}
			}
		}
		if ( $need_responsive ) {
			foreach ( $responsive as $k => $v ) {
				...
				if ( ! empty( $sizes[ $media ] ) ) {
					$output .= "\n\n"
							...
							. strip_tags( sprintf( __( '/* SASS Suffix: --%s */', 'easyeat' ), $k ) )
							. "\n"
							. "@media {$media} {\n"
								. $sizes[ $media ]
							. "\n}\n";
			...
			if ( count( $sizes ) > 0 ) {
				$output .= "\n\n"
						. strip_tags( __( '/* Unknown Suffixes: */', 'easyeat' ) );
		...
		if ( '' != $output ) {
			...
			if ( empty( $file_dir ) && strpos( $to, '-full.css' ) !== false ) {
				$file_dir = easyeat_get_file_dir( str_replace( '-full.css', '.css', $to ) );
				if ( ! empty( $file_dir ) ) {
			...
			easyeat_fpc( $file_dir,
				...
				. strip_tags( __("ATTENTION! This file was generated automatically! Don't change it!!!", 'easyeat') ) 
				. "\n----------------------------------------------------------------------- */\n"
				. apply_filters( 'easyeat_filter_css_output', apply_filters( 'easyeat_filter_prepare_css', $output, true ), $to )
			);
		}
	}
}

if ( ! function_exists( 'easyeat_filter_merge_css' ) ) {
	add_filter( 'easyeat_filter_merge_css', 'easyeat_filter_merge_css' );
	/**
	 ...
	 */
	function easyeat_filter_merge_css( $css ) {
		return str_replace( '../../../../', '../../../', $css);
	}
}

if ( ! function_exists( 'easyeat_merge_sass' ) ) {
	/**
	 ...
	 */
	function easyeat_merge_sass( $to, $list, $need_responsive = false, $root = '../' ) {
		if ( $need_responsive ) {
			$responsive = apply_filters( 'easyeat_filter_responsive_sizes', easyeat_storage_get( 'responsive' ) );
		}
		$sass                = array(
		...
		foreach ( $list as $f ) {
			...
			if ( $add ) {
				$sass['import'] .= apply_filters( 'easyeat_filter_sass_import', "{$sass_import} \"{$root}{$f}\";\n", $f );
				$save            = true;
			}
		}
		if ( $save ) {
			...
			easyeat_fpc( easyeat_get_file_dir( $to ), apply_filters( 'easyeat_filter_sass_output', $output, $to ) );
		}
	}
}
...
if ( ! function_exists( 'easyeat_get_domain_from_url' ) ) {
	/**
	 ...
	 */
	function easyeat_get_domain_from_url( $url ) {
		$pos = strpos( $url, '//' );
		if ( $pos !== false ) {
...
if ( ! function_exists( 'easyeat_add_protocol' ) ) {
	/**
	 ...
	 */
	function easyeat_add_protocol( $url ) {
		return substr( $url, 0, 2 ) == '//' ? easyeat_get_protocol() . ':' . $url : $url;
	}
}

if ( ! function_exists( 'easyeat_remove_protocol_from_url' ) ) {
	/**
	 ...
	 * @return string         A processed string.
	 */
	function easyeat_remove_protocol_from_url( $url, $complete = true ) {
		return preg_replace( '/(http[s]?:)?' . ( $complete ? '\\/\\/' : '' ) . '/', '', $url );
	}
}

if ( ! function_exists( 'easyeat_add_to_url' ) ) {
	/**
	 ...
	 */
	function easyeat_add_to_url( $url, $prm ) {
		if ( is_array( $prm ) && count( $prm ) > 0 ) {
			...
			$params = easyeat_array_merge( $params, $prm );
			$url = $parts[0];
			$separator = '?';
...
if ( ! function_exists( 'easyeat_is_url' ) ) {
	/**
	 ...
	 */
	function easyeat_is_url( $url ) {
		return is_string( $url ) && ( strpos( $url, '//' ) === 0 || strpos( $url, '://' ) !== false );
	}
}

if ( ! function_exists( 'easyeat_is_external_url' ) ) {
	/**
	 ...
	 */
	function easyeat_is_external_url( $url ) {
		return easyeat_is_url( $url ) && strpos( $url, easyeat_remove_protocol_from_url( apply_filters( 'trx_addons_filter_home_url', get_home_url() ), true ) ) === false;
	}
}

if ( ! function_exists( 'easyeat_url_to_local_path' ) ) {
	/**
	 ...
	 */
	function easyeat_url_to_local_path( $url ) {
		$path = '';
		...
		$locations = array(
			'uploads' => array(
				...
				'url' => easyeat_remove_protocol_from_url( $upload_info['baseurl'] )
				),
			'child' => array(
				'dir' => EASYEAT_CHILD_DIR,
				'url' => easyeat_remove_protocol_from_url( EASYEAT_CHILD_URL )
				),
			'theme' => array(
				'dir' => EASYEAT_THEME_DIR,
				'url' => easyeat_remove_protocol_from_url( EASYEAT_THEME_URL )
				)
			);
